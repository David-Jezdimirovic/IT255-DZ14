...
export class UpdateroomComponent implements OnInit {
  ...
  private _getRoomTypes() {
    ...
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append('token', localStorage.getItem('token'));
    this.http.get('http://localhost:8080/IT255-DZ14/getroomtypes.php', {headers: headers}).subscribe((data): any => {
  ...
  public updateRoom() {
    ...
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append("token",localStorage.getItem("token"));
    this.http.post('http://localhost:8080/IT255-DZ14/updateRoom.php', data, { headers:headers }).subscribe( data => {
