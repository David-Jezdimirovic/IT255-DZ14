...
export class DodavanjeSobeComponent implements OnInit {
  ...
  private _getRoomTypes() {
    ...
    this._http.get('http://localhost:8080/IT255-DZ14/getroomtypes.php', {headers: headers}).subscribe((data): any => {
  ...
  public dodajSobu() {
    ...
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append("token",localStorage.getItem("token"));
    this._http.post('http://localhost:8080/IT255-DZ14/dodavanjeSobe.php',data, { headers:headers }).subscribe( data => {
  ...
  public addRoomType() {
    ...
    this._http.post('http://localhost:8080/IT255-DZ14/addroomtype.php', data, { headers: headers}).subscribe((result) => {
