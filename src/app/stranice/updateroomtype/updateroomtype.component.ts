...
export class UpdateroomtypeComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe((params: Params) => { 
      ...
      let headers = new Headers();
       ...
       headers.append("token",localStorage.getItem("token"));
        this.http.get('http://localhost:8080/IT255-DZ14/getroomtype.php?id='+id,{headers:headers}).subscribe(data => {
   ...
   public updateRoomType() {
    ...
    headers.append('Content-Type', 'application/x-www-form-urlencoded');
    headers.append("token",localStorage.getItem("token"));
    this.http.post('http://localhost:8080/IT255-DZ14/updateRoomType.php', data, { headers:headers }).subscribe( data => {
